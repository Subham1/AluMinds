...
export default function  Dashboard  ()  {
    ...
    const handlePredict = () => {
        ...
        axios.post('http://localhost:5000/predict', numericInputData, {
            headers: {
    ...
    const handleOptimize = () => {
        const numericOptimizeData = Object.fromEntries(
            Object.entries(optimizeData).map(([key, value]) => [key, parseFloat(value)])
        );
        ...
        axios.post('http://localhost:5000/optimize', numericOptimizeData, {
            headers: {
                'Content-Type': 'application/json'
            }
        })
  ...
  'Casting Temperature': parseFloat(data['Casting Temperature']).toFixed(2),
  'Cooling Water Temperature': parseFloat(data['Cooling Water Temperature']).toFixed(2),
  'Casting Speed': parseFloat(data['Casting Speed']).toFixed(2),
  'Cast Bar Entry Temperature': parseFloat(data['Cast Bar Entry Temperature']).toFixed(2),
  'Emulsion Temperature': parseFloat(data['Emulsion Temperature']).toFixed(2),
  'Emulsion Pressure': parseFloat(data['Emulsion Pressure']).toFixed(2),
  'Emulsion Concentration': parseFloat(data['Emulsion Concentration']).toFixed(2),
  'Rod Quench Water Pressure': parseFloat(data['Rod Quench Water Pressure']).toFixed(2)
            });
    ...
    const sumOfChemicals = parseFloat(inputData['Al%'] || 0) 
            ...
            <div className="other-content">
              <Parameters inputData={inputData}/>
                        <div className="dashboard-row">
                            ...
                            />
                            <Visualization
                                conductivity={output.Conductivity ? output.Conductivity.toString().substring(0, 5) : 'N/A'}
                                elongation={output.Elongation ? output.Elongation.toString().substring(0, 5) : 'N/A'}
